...
function Products(){
    ...
    const fetchProducts = async (page=1) => {
        ...
        try {
            ...
            setFilteredProduct(res.data.results);
...
useEffect(() => {
    ...
    useEffect(() => {
        if (!urlLoaded) return;
        fetchProducts(1);
    }, [urlLoaded,selectedBrand, selectedType]);


   console.log("Product in profuct page",filterProducts);
   
    

    return (
        <div className="relative bg-gradient-to-br from-gray-900 via-gray-950 to-[#0b0b0b]
 p-4 rounded-lg">
                     ...
                     <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-10 mt-10">
                        ...
                        filteredProducts.map((product)=>(
                            ...
                            <div className="col-span-full text-center text-gray-400 py-10">
                                <p>{` ${selectedType || ""}s in ${selectedBrand || ""} brand are not available`.trim() || "No products found"}</p>
                            </div>
                        )}
                     </div>
            ...
            Next →
                       ...
                       {selectedProduct && (
                            <div className={`fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 transition-all duration-300 ${isModelOpen ? 'backdrop-blur-sm opacity-100' : 'backdrop-blur-0 opacity-0'}`} >
                                <div  className={`relative max-w-full sm:max-w-4xl w-full mx-2 sm:mx-0 bg-black/30 border border-white/20 rounded-2xl overflow-hidden backdrop-blur-lg transition-all duration-300 transform ${isModelOpen ? 'scale-100 opacity-100' : 'scale-90 opacity-0'}`}>
                                ...
                                <div className="flex flex-col md:flex-row ">
                                    ...
                                    <div className="md:w-1/2 p-3 sm:p-6 text-white">
                                        ...
                                        <p className="text-white/80 text-sm sm:text-base mb-4 sm:mb-20 font-medium">{selectedProduct.description || "Product description goes here."}
                                        </p>
                                                    ...
                                                    onClick={()=>ToggleWishList(selectedProduct)}>
                                                        ...
                                                        <span className="relative z-10 flex items-center justify-center">
                                                            <FaHeart  className={`text-xl transition duration-300 hover:scale-110 ${
                                                                wishlist.some(item => item.product.id === selectedProduct.id) ? 'text-red-500 fill-red-500' : 'text-white'
                                                                }`}/>
                                                        </span>
